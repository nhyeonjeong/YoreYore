...
북마크는 Realm에 저장이 되도록 했고, 북마크 화면의 collectionview는 DiffableSource로 구현.
하지만 레시피 상세페이지에서 북마크 해제한 후 북마크화면으로 넘어가면 collectionview를 그리는 과정에서 "Object has been deleted or invalidated" 런타임오류
DiffableSource에서 apply메서드로 뷰를 그릴 때는 참조형식으로 데이터들에 접근을 하려고 해서 이미 삭제된 데이터에 접근하려고 할 때 런타임오류가 발생

1-2) 해결

Realm에서 가져온 데이터는 class타입으로 되어있다 이 데이터를 바로 사용하지 않고 
...
## 🧑🏼‍🍳기술회고
Realm에서 가져온 데이터에 DTO를 적용할 때 realm에 접근해 데이터를 가져오는 레포지터리 함수내에서 진행했었는데 
